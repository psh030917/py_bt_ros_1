...
class UpdateFireState(ConditionWithROSTopics):
    ...
    - 화재 감지 기준:
      ...
      - temp_threshold: XML 파라미터 (기본값: 600) - temp < 이 값이면 화재
    - blackboard에 저장:
    ...
    def _predicate(self, agent, blackboard) -> bool:
        ...
        if "fire_sensor" in cache:
            try:
                ...
                # 화재 감지 로직: flame >= flame_threshold or temp < temp_threshold
                ...
                agent.ros_bridge.node.get_logger().debug(
                    ...
                    f'fire_detected={fire_detected} (기준: flame>={flame_threshold}, temp<{temp_threshold})'
                )
                return True
...
class StopRobot(Node):
    ...
    async def run(self, agent, blackboard):
        ...
        try:
            ...
            self.ros.node.get_logger().error(f'StopRobot: 정지 명령 발행 실패 - {e}', exc_info=True)
            self.status = Status.FAILURE
            return self.status
